...
void Stage3Barrier::Initialize(Model* model, Vector3 position)
{
	assert(model);
	model_ = model;
...
void Stage3Barrier::Draw(ViewProjection& viewProjection)
{
	if (isDead_ == false)
	{
...
void Stage3Barrier::OnCollision()
{
	isOnCollision_ = true;
}

void Stage3Barrier::Reset()
{
	isDead_ = false;
	isOnCollision_ = false;
...
Vector3 Stage3Barrier::GetPosition()
{
	Vector3 worldPos;
...
Vector3 Stage3Barrier::GetScale()
{
	Vector3 worldScale;
