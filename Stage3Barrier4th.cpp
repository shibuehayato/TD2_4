...
void Stage3Barrier4th::Initialize(Model* model, Vector3 position)
{
	assert(model);
	model_ = model;
...
void Stage3Barrier4th::Draw(ViewProjection& viewProjection)
{
	if (isDead_ == false)
	{
...
void Stage3Barrier4th::OnCollision()
{
	isOnCollision_ = true;
}

void Stage3Barrier4th::Reset()
{
	isDead_ = false;
	isOnCollision_ = false;
...
Vector3 Stage3Barrier4th::GetPosition()
{
	Vector3 worldPos;
...
Vector3 Stage3Barrier4th::GetScale()
{
	Vector3 worldScale;
